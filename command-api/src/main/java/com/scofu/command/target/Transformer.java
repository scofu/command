...
public interface Transformer<T> extends Feature, Predicate<Type> {
   ...
   */
  Result<T> transform(
      Command command, Parameter<T> parameter, Parameters parameters, Arguments arguments);
  ...
  default Stream<String> suggest(
      Command command, Parameter<T> parameter, Parameters parameters, Result<String> argument) {
    return Stream.empty();
  }
