...
public class PermissionValidator implements Validator {
  ...
  public <T, R> boolean validate(Context context, Node<T, R> node) {
    return context.expand(Permission.HOLDER_IDENTIFIER)
        .flatMap(permissionHolder -> node.expand(Permission.PERMISSION_IDENTIFIER)
            .map(permissionHolder::hasPermission))
