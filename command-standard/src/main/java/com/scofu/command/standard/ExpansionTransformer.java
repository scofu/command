...
final class ExpansionTransformer implements Transformer<Expansion> {
  ...
  @Override
  public Result<Expansion> transform(
      Command command, Parameter<Expansion> parameter, Parameters parameters, Arguments arguments) {
    ...
    return Result.value(
        Optional.ofNullable(command.context().expansions().get(identifier))
